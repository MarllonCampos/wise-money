...
export function InputFloatingLabel({ label, style, type = "email-address", placeholder, secure = false, textChange, ...props }: Props) {
        ...
        >
            ...
            {secure &&
                <TouchableOpacity onPress={() => setIsPasswordShowing(prevState => !prevState)}>
                   { (isPasswordShowing
                    ? <EyesOpen style={styles.eyes} />
                    : <EyesClosed fontSize={12} style={styles.eyes}/>
                    )}
                </TouchableOpacity>
            }
